...
import { FONT, KBOX, KCYL, MergeStatic, canvasTexture, fitFont, kgeo, km } from '../trainer/kit';
  ...
  return (
    <MergeStatic>
      <group position={position}>
        ...
        <mesh
          geometry={kgeo('ms:cpt-coil', () => new THREE.CapsuleGeometry(0.019, 0.034, 6, 16).rotateZ(Math.PI / 2))}
          material={coil}
          scale={[1, 1.55, 1.25]}
          position={[0, 0, 0.046]}
          castShadow
          ...
          <group key={x} position={[x, 0.065, 0.03]}>
            ...
            {[-1, 1].map((sz) => (
              <mesh
                key={sz}
                geometry={KCYL()}
                material={km.metal('#c9b27a', 0.3)}
                rotation={[Math.PI / 2, 0, 0]}
                scale={[0.0104, 0.005, 0.0104]}
                position={[0, 0, sz * 0.0145]}
              />
            ))}
          </group>
        ))}
