...
import type { MotorProps, Placement } from '../../contracts';
import {
  box,
  canvasTex,
  Cable,
...
const HOT = new THREE.Color('#ff3c06');
...
function finnedFrameGeo(frame: MotorFrame) {
  return geo(`motorFrame:${frame}`, () => {
    const s = MOTOR_FRAMES[frame];
    const R = s.bodyR;
    const Rt = R + s.finH;
    const gaps: [number, number][] = [
      [-0.3, 0.3], // conduit box pad (+X)
      [TAU * 0.62, TAU * 0.88], // feet (bottom)
    ];
    const inGap = (a: number) => gaps.some(([a0, a1]) => {
    ...
    const baseHalf = (R * 0.075) / 2 / R;
    const tipHalf = (R * 0.036) / 2 / Rt;
    ...
    for (const a of fins) {
      ...
      P(R, start);
      P(R + s.finH * 0.12, a - baseHalf * 0.85);
      P(Rt - s.finH * 0.12, a - tipHalf);
      P(Rt, a - tipHalf * 0.4);
      P(Rt, a + tipHalf * 0.4);
      P(Rt - s.finH * 0.12, a + tipHalf);
      P(R + s.finH * 0.12, a + baseHalf * 0.85);
    ...
    const bt = 0.003;
    const g = new THREE.ExtrudeGeometry(shape, { depth: s.bodyLen - 2 * bt, bevelEnabled: true, bevelThickness: bt, bevelSize: 0.0015, bevelSegments: 2, curveSegments: 4 });
...
export interface MotorBodyProps {
   ...
   *  'gland' = cable gland on the box end with a black power cable running toward local -X for `conduitDrop`. */
...
export function MotorBody({
  ...
  // Per-instance cast material (overload glow).
  const castMat = useMemo(() => {
  ...
  useFrame(({ clock }) => {
    if (!getOverloaded) return;
    const ov = getOverloaded();
    const t = clock.elapsedTime;
    castMat.emissive.copy(HOT);
    castMat.emissiveIntensity = ov ? 0.22 + 0.18 * Math.sin(t * 4) : 0;
  });
  ...
  const npTex = nameplateTex(plate);
  const hubR = s.shaftR * 2.2;
  const hubL = s.shaftLen * 0.55;

  return (
    <group>
      {/* finned frame */}
      <mesh geometry={finnedFrameGeo(frame)} material={castMat} castShadow receiveShadow />
      {/* conduit box pad */}
      ...
      <group rotation={[0, 0, 0.35]}>
        <mesh position={[Rf + 0.0006, 0, (zf + zb) / 2 + 0.012 * sc]} rotation={[0, Math.PI / 2, 0]}>
          <planeGeometry args={[0.045 * sc, 0.022 * sc]} />
          <meshStandardMaterial map={arrowDecalTex()} transparent roughness={0.5} />
        </mesh>
      </group>
      {[0, 1, 2].map((i) => {
        ...
        return (
          <PanScrew
            key={i}
            d={0.004 * sc}
            position={[Math.cos(a) * Rf, Math.sin(a) * Rf, zb + s.ndeLen * 0.25]}
            rotation={[0, 0, 0]}
          />
        );
      })}
      ...
      <group ref={rotor}>
        ...
        {shaft && (
          <>
            ...
            <mesh
              geometry={box(s.shaftR * 0.5, s.shaftR * 0.3, s.shaftLen * 0.78)}
              material={fm.chrome()}
              position={[0, s.shaftR * 0.93, s.shaftLen * 0.52]}
            />
          </>
        )}
        {shaft && coupling === 'hub' && (
          <group position={[0, 0, s.shaftLen - hubL]}>
            <mesh geometry={cylZ(hubR, hubL, 32)} material={fm.cast('#4b4f54', 0.4)} position={[0, 0, hubL / 2]} castShadow />
            {/* jaws + elastomer spider */}
            {[0, 1, 2, 3, 4, 5].map((i) => (
              <mesh
                key={i}
                geometry={geo(`jaw:${frame}`, () => {
                  const sh = new THREE.Shape();
                  const a0 = -TAU / 12 + 0.02;
                  const a1 = TAU / 12 - 0.02;
                  sh.absarc(0, 0, hubR * 0.98, a0, a1, false);
                  sh.absarc(0, 0, hubR * 0.42, a1, a0, true);
                  return new THREE.ExtrudeGeometry(sh, { depth: hubL * 0.45, bevelEnabled: false, curveSegments: 6 });
                })}
                material={i % 2 === 0 ? fm.cast('#4b4f54', 0.4) : fm.plastic('#f08a1c', 0.55)}
                position={[0, 0, hubL]}
                rotation={[0, 0, (i * TAU) / 6]}
              />
            ))}
            {/* paint stripe to see rotation */}
      ...
      <group position={[boxX, 0.004 * sc, boxZ]}>
        ...
        {[
          ...
          <PanScrew
            key={i}
            d={0.0045 * sc}
            position={[boxW / 2 + 0.006 * sc, a! * boxH * 0.4, b! * boxD * 0.42]}
            rotation={[0, Math.PI / 2, 0]}
          />
        ))}
        {/* threaded hub + liquid-tight connector (conduit entry pointing down) */}
        {conduit === 'gland' && (
          <group position={[0, -boxH * 0.15, -boxD / 2]}>
            ...
            <Cable
              ...
              points={[
                ...
                [0, 0, -0.05 * sc],
                [-0.03 * sc, 0, -0.08 * sc],
                [-conduitDrop * 0.5, 0, -0.1 * sc],
                [-conduitDrop, 0.01, -0.11 * sc],
              ]}
            />
          </group>
        )}
      ...
      {/* nameplate (on the fin tips) */}
      <group rotation={[0, 0, nameplateAngle]}>
        <mesh position={[Rt + 0.0012, 0, -s.deLen - s.bodyLen * 0.55]} rotation={[nameplateFlip ? Math.PI : 0, Math.PI / 2, 0]}>
          <planeGeometry args={[0.105 * sc, 0.062 * sc]} />
          <meshStandardMaterial map={npTex} metalness={0.6} roughness={0.35} />
        </mesh>
      </group>

      {/* lifting eye bolt */}
      {liftingEye && <group position={[0, Rt, -s.deLen - s.bodyLen * 0.45]}>
        <mesh geometry={cylY(0.012 * sc, 0.012 * sc, 16)} material={castMat} position={[0, -0.004 * sc, 0]} />
...
export interface MotorExtraProps {
  ...
  mount?: 'foot' | 'flange';
  /** Jaw-coupling hub on the shaft (default true for foot mount). */
  coupling?: boolean;
...
export interface GearMotorProps extends Placement {
  ...
  getOverloaded?: () => boolean;
  /** Torque arm length (m) hanging down from the gearbox; 0 = none. */
  torqueArm?: number;
}
...
export function GearMotor({
  ...
  const { depth, inputZ, flangeX, boreR } = GEARMOTOR;
  ...
  return (
    <group position={position} rotation={rotation} scale={scale}>
      ...
      <group position={[sgn * 0.074, 0, inputZ]}>
        <mesh geometry={geo('gmAdapter', () => {
          const g = new THREE.CylinderGeometry(0.058, 0.062, 0.03, 40);
          g.rotateZ(Math.PI / 2);
          return g;
        })} material={cast} position={[sgn * 0.012, 0, 0]} castShadow />
        <mesh geometry={geo('gmFlange', () => {
          const g = new THREE.CylinderGeometry(0.074, 0.074, 0.012, 40);
          g.rotateZ(Math.PI / 2);
          return g;
        })} material={cast} position={[sgn * (flangeX - 0.074 - 0.006), 0, 0]} />
      </group>
      {/* oil plugs & breather */}
      ...
      {/* nameplate */}
      <mesh position={[sgn * 0.004, 0.095, depth + 0.0003]}>
        <planeGeometry args={[0.06, 0.026]} />
        <meshStandardMaterial map={gearNameplateTex(ratio)} metalness={0.5} roughness={0.4} />
      </mesh>
      {/* rotating output collar (machine side) */}
      <group ref={out} position={[0, 0, -0.006]}>
      ...
      {torqueArm > 0 && (
        <group position={[0, -0.075, 0.018]}>
          <mesh geometry={box(0.03, torqueArm, 0.01)} material={fm.sheet('#2e3134', 0.5)} position={[0, -torqueArm / 2, 0]} castShadow />
          <mesh geometry={cylZ(0.016, 0.03, 20)} material={fm.rubber('#1a1a1a')} position={[0, -torqueArm, -0.004]} />
          <mesh geometry={box(0.05, 0.05, 0.006)} material={fm.zinc()} position={[0, -torqueArm, -0.018]} />
          <HexBolt d={0.008} position={[0, -torqueArm, 0.011]} />
        </group>
      )}
      {/* motor */}
