...
| **Learning aids** | A guided first-rung tour. Live objective checks while you edit (graded only when you press Verify & Test). "Try it now" tips. Failed-test replays that fly the camera to the device under test, pause at the failure and show an expected-vs-actual trace. Hovering I/O rows or tag chips highlights the device in 3D. A picture-in-picture twin gives the ladder more room while you edit. |
