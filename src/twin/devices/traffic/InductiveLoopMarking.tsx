/**
 * Inductive vehicle-detector loop as seen on the pavement: 1/4" saw-cut slots filled with black
 * loop sealant — a rectangle (or quadrupole with a center cut) with 45° chamfered corners and small
 * saw over-cuts, plus the lead-in cut (twisted pair) running to the curb / pull box. When `getActive`
 * returns true the slots glow softly (a detection "game" hint) and the detection zone is tinted.
 *
...
export function InductiveLoopMarking({
  ...
  const halo = cutGeometry(key + ':halo', list, CUT_W * 5);
  ...
  useFrame(({ clock }, dt) => {
    ...
    coreMat.emissiveIntensity = l * 2.2 * pulse;
    glowMat.opacity = l * 0.35 * pulse;
    zoneMat.opacity = l * 0.08;
  });
  // A faint patch of fresher asphalt / sealant bleed around the cuts
  return (
    <group position={position} rotation={rotation} scale={scale}>
      <mesh geometry={cuts} material={coreMat} position={[0, 0.0015, 0]} receiveShadow />
      <mesh geometry={halo} material={glowMat} position={[0, 0.002, 0]} renderOrder={3} />
      <mesh geometry={planeGeo(length, width)} material={zoneMat} position={[0, 0.0012, 0]} rotation={[-Math.PI / 2, 0, 0]} renderOrder={2} />
    </group>
  );
}
