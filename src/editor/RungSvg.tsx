...
function BranchView({ n, sel }: { n: BranchLayout; sel: LadderSelection | undefined }) {
  const last = n.legYs[n.legYs.length - 1]!;
  const selected = sel?.elementId === n.id;
  return (
    <g className="ld-br" data-el={n.id}>
      {selected && <rect className="ld-brsel" x={n.x - 6} y={n.top - 2} width={n.x2 - n.x + 12} height={n.bottom - n.top + 4} />}
...
function RungSvgImpl(p: RungSvgProps) {
  ...
  return (
    ...
    >
      ...
      {!p.bare && (
        <>
          <rect className="ld-margin" x={0} y={0} width={L.railL - 10} height={L.height} />
          <line className="ld-margin-edge" x1={L.railL - 10.5} y1={0} x2={L.railL - 10.5} y2={L.height} />
          <rect className="ld-numbox" x={L.railL - 46} y={numY - 10} width={32} height={19} rx={4} data-rungnum="" />
          <text className="ld-num" x={L.railL - 18} y={numY + 4} textAnchor="end">
            {p.index}
          </text>
          {(errs.length > 0 || warns.length > 0) && (
            <g className={cn('ld-marker', errs.length > 0 ? 'e' : 'w')} data-marker="">
              <rect x={5} y={numY - 7.5} width={14} height={15} />
              <text x={12} y={numY + 3.5} textAnchor="middle">
                {errs.length > 0 ? 'e' : 'w'}
              </text>
            </g>
          )}
      ...
      {L.nodes.map((n) =>
        n.kind === 'instr' ? (
          ...
          <BranchView key={n.id} n={n} sel={sel} />
        ),
      )}
      {p.dropGap && (
        <g>
...
export function EndRung({ width, zoom, railL, railR, children }: { width: number; zoom: number; railL: number; railR: number; children?: ReactNode }) {
