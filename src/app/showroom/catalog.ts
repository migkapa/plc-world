...
export const SHOWROOM_DEVICES: ShowroomDevice[] = [
  ...
  },
  {
    ...
    specs: [
      ...
      { label: 'Embedded port', value: '1 × RJ45 10/100/1000 Mbps EtherNet/IP (on the underside)' },
    ...
    hotspots: [
      { label: 'Status display', at: [0, MOD_Y + 0.1245, 0.1405], text: 'Scrolling **4-character display**: mode (`Rem Run`, `Prog`), the IP address, and major fault codes like `Major Fault T04:C20`.' },
      { label: 'RUN · FORCE · SD · OK', at: [-0.005, MOD_Y + 0.1105, 0.1405], text: '**RUN** green = scanning logic. **FORCE** amber = forces enabled (flashing = installed, not enabled). **SD** = card activity. **OK** green = healthy, flashing red = major fault.' },
      { label: 'Key switch', at: [0, MOD_Y + 0.0848, 0.1425], text: '**RUN / REM / PROG** key. In REM the programming software can change the mode remotely. Try it in the demo panel!' },
      { label: 'SD card', at: [0, MOD_Y + 0.0585, 0.1405], text: 'Behind the door: **SD card** slot for storing the project and firmware (load on power-up / on corrupt memory).' },
      { label: 'USB port', at: [0, MOD_Y + 0.0392, 0.1405], text: '**USB 2.0** device port for local programming and firmware updates — not for permanent HMI connections.' },
      { label: 'NET · LINK', at: [-0.005, MOD_Y + 0.0205, 0.1405], text: 'Status of the embedded **EtherNet/IP** port: NET = IP/CIP connections, LINK = cable link and traffic.' },
      { label: 'Ethernet port', at: [0, MOD_Y - 0.002, 0.129], normal: [0, -1, 0.3], text: 'The **1 Gb RJ45** jack sits on the underside, so the patch cable exits downward into the wire duct.' },
    ],
